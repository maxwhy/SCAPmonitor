...
# SCAPmonitor18 allows continuous monitoring of a Ubuntu 18.04 system, using
# OpenSCAP to scan and remediate the system when a change that can affect the
# security is detected.

# SCAPmonitor18 requires the user to input the path of a specific file included
# in SCAP Security Guide (SSG): ssg-ubuntu1804-ds.xml
if [ $# != 1 ];
then
  echo "Usage: bash SCAPmonitor18.sh filepath"
  echo "Where filepath refers to the ssg-ubuntu1804-ds.xml of SSG"
  exit
fi
FILEPATH="$1"
